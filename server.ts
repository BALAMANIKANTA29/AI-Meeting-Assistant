...
// POST /api/upload - Audio uploads, recordings, simulations, or manual notes
...
Respond ONLY with raw valid JSON. Do not include markdown code block formatting or backticks.`,
    ...
    if (transcriptObj.length === 0) {
      if (manualNotes && manualNotes.trim().length > 0) {
        ...
        const bulletText = lines.map((l: string) => (l.trim().startsWith("-") || l.trim().startsWith("*") ? l.trim() : `- ${l.trim()}`)).join("\n");
        transcriptObj = [
          {
            speaker: "Manual Notes",
            text: bulletText,
